...
def normalizing_input_data(coor_data):
    ...
    norm_matrix = np.matrix([[sx,0,-sx*x_avg],[0,sy,-sy*y_avg],[0,0,1]])
    return norm_matrix
...
def jacobian(H,pic_coor,real_coor):
    ...
    for i in range(len(real_coor)):
        ...
        w2 = w*w
        J.append(np.array([real_coor[i][0] / w, real_coor[i][1] / w, 1 / w, 0, 0, 0,
                           -sx * real_coor[i][0] / w2, -sx * real_coor[i][1] / w2, -sx / w2]))
        J.append(np.array([0, 0, 0, real_coor[i][0] / w, real_coor[i][1] / w, 1 / w,
                           -sy * real_coor[i][0] / w2, -sy * real_coor[i][1] / w2, -sx / w2]))
    return np.array(J)
